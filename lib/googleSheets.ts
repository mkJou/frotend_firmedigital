...
/**
 * Configura la autenticación con Google usando variables de entorno
 */
async function getAuthClient() {
  try {
    ...
    const credentials = {
      ...
      project_id: 'stellar-arcadia-454920-e0',
    ...
    // Verificar que las credenciales necesarias existan
    if (!process.env.GOOGLE_PRIVATE_KEY || !process.env.GOOGLE_CLIENT_EMAIL) {
      throw new Error('Variables de entorno para credenciales de Google no encontradas');
    }
